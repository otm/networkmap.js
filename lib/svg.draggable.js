...
SVG.extend(SVG.Element, {
  ...
  draggable: function(constraint) {
      ...
      if (element.startEvent) {
        ...
        if (element._offset) {
          ...
          y -= element._offset.y
			...
			if (constraint.grid && constraint.grid.x != null)
				x = constraint.grid.x * Math.round(x / constraint.grid.x)
				
			if (constraint.grid && constraint.grid.y != null)
				y = constraint.grid.y * Math.round(y / constraint.grid.y)
        
      if (constraint.fn){
        p = constraint.fn({x: x, y: y});
        x = p.x;
        y = p.y;
      }
        ...
        if (delta.x > 5 || delta.y > 5)
           event.target.instance.data('dragged', true);
    ...
    /* when dragging ends */
      ...
      setTimeout(function(){
        event.target.instance.data('dragged', false);
      }, 10);
